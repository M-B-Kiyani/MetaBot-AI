      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ requestCount, endpoint, method }) => {
            ...
            // Make requests sequentially with small delays to allow rate limiter to work properly
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ endpoint, excessiveRequestCount }) => {
            ...
            // Make excessive requests sequentially to ensure proper rate limiting
            for (let i = 0; i < excessiveRequestCount; i++) {
              const response = await request(app).get(endpoint);
              responses.push(response);

              // Small delay between requests
              if (i < excessiveRequestCount - 1) {
                await new Promise((resolve) => setTimeout(resolve, 10));
              }
            }
            ...
            // Verify rate-limited responses have proper error structure
            const rateLimitedResponses = responses.filter(
              (r) => r.status === 429
            );
            rateLimitedResponses.forEach((response) => {
