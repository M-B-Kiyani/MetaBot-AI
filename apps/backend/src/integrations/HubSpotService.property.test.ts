    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (booking: Booking) => {
          ...
          const mockContactId =
            'contact-' + Math.random().toString(36).substr(2, 9);
          ...
          const contactData =
            HubSpotService.extractContactDataFromBooking(booking);
      ...
      fc.asyncProperty(
        fc.record({
          ...
          name: fc
            ...
            .filter((name) => name.trim().length > 0), // Ensure non-empty after trim
          email: fc.emailAddress(),
          phone: fc.option(fc.string({ minLength: 10, maxLength: 15 })),
          inquiry: fc.option(fc.string({ minLength: 1, maxLength: 500 })),
          startTime: fc.date({
